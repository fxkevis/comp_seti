...
bool ssl_send_msgex(SSL *ssl, uint8_t type, const char* sender, const char* receiver, const char* text, const char* dst_ip = "127.0.0.1") 
{
    MessageEx msg;
    ...
    std::cout << "[Security][ENC] SSL_write " << type_name(type) << std::endl;
    if (!ssl_send_all(ssl, &msg.length, sizeof(uint32_t))) return false;
    if (!ssl_send_all(ssl, &msg.type, sizeof(uint8_t))) return false;
    if (!ssl_send_all(ssl, &msg.msg_id, sizeof(uint32_t))) return false;
    if (!ssl_send_all(ssl, msg.sender, MAX_NAME)) return false;
    if (!ssl_send_all(ssl, msg.receiver, MAX_NAME)) return false;
    if (!ssl_send_all(ssl, &msg.timestamp, sizeof(time_t))) return false;
    if (!ssl_send_all(ssl, msg.payload, plen + 1)) return false;
    return true;
}

bool ssl_send_ack(SSL *ssl, uint32_t ack_id, const char* dst_ip) 
{
    char buf[32];
    snprintf(buf, sizeof(buf), "%u", ack_id);
    std::cout << "[Transport][ACK] send MSG_ACK (id=" << ack_id << ")" << std::endl;
    return ssl_send_msgex(ssl, MSG_ACK, "SERVER", "", buf, dst_ip);
}

bool ssl_recv_msgex(SSL *ssl, MessageEx& msg, const char* src_ip) 
{
...
void broadcast_ssl(const char* text, SSL *sender_ssl, const char* sender_nick) 
{
    pthread_mutex_lock(&clients_mutex);
    for (size_t i = 0; i < clients.size(); i++)
        if (clients[i]->ssl != sender_ssl && clients[i]->ssl != NULL)
            ssl_send_msgex(clients[i]->ssl, MSG_TEXT, sender_nick, "", text, clients[i]->ip);
    pthread_mutex_unlock(&clients_mutex);
}

bool send_private_online_ssl(const char* target, const char* sender, const char* text, bool is_offline) 
{
    pthread_mutex_lock(&clients_mutex);
    for (size_t i = 0; i < clients.size(); i++) 
    {
        if (clients[i]->authenticated && strcmp(clients[i]->nickname, target) == 0 && clients[i]->ssl != NULL) 
        {
            ...
            ssl_send_msgex(clients[i]->ssl, MSG_PRIVATE, sender, target, buf, clients[i]->ip);
            pthread_mutex_unlock(&clients_mutex);
            return true;
        }
    }
...
void deliver_offline(const char* nick, SSL *ssl, const char* ip) 
{
    ...
    for (size_t i = 0; i < offline_queue.size(); i++) 
    {
        if (strcmp(offline_queue[i].receiver, nick) == 0) 
        {
            ...
            if (send_private_online_ssl(om.receiver, om.sender, om.text, true)) 
            {
...
void* worker(void*) 
{
    ...
    while (true) 
    {
        ...
        if (!ssl) {
            std::cerr << "[Security][TLS] SSL object creation failed" << std::endl;
            close(fd);
            continue;
        }
        ...
        MessageEx msg;
        if (!ssl_recv_msgex(ssl, msg, ip) || msg.type != MSG_HELLO) 
        {
        ...
        ssl_send_msgex(ssl, MSG_WELCOME, "SERVER", "", welcome_buf, ip);

        if (!ssl_recv_msgex(ssl, msg, ip) || msg.type != MSG_AUTH) 
        {
            ssl_send_msgex(ssl, MSG_ERROR, "SERVER", "", "Authentication required", ip);
            SSL_shutdown(ssl);
            SSL_free(ssl);
            close(fd);
            continue;
        ...
        if (strlen(nick) == 0) 
        {
            ssl_send_msgex(ssl, MSG_ERROR, "SERVER", nick, "Nickname cannot be empty", ip);
            SSL_shutdown(ssl);
            SSL_free(ssl);
            close(fd);
            continue;
        ...
        if (taken) 
        {
            ssl_send_msgex(ssl, MSG_ERROR, "SERVER", nick, "Nickname already taken", ip);
            SSL_shutdown(ssl);
            SSL_free(ssl);
            close(fd);
            continue;
        ...
        ssl_send_msgex(ssl, MSG_SERVER_INFO, "SERVER", nick, info, ip);
        broadcast_ssl(info, ssl, "SERVER");
        deliver_offline(nick, ssl, ip);

        while (true) {
            if (!ssl_recv_msgex(ssl, msg, ip)) 
            {
            ...
            if (msg.type == MSG_TEXT) 
            {
                if (check_dedup(c, msg.msg_id)) 
                {
                    ...
                    ssl_send_ack(ssl, msg.msg_id, ip);
                    continue;
                }
                ...
                broadcast_ssl(display, ssl, nick);
                ssl_send_ack(ssl, msg.msg_id, ip);
            }
            else if (msg.type == MSG_PRIVATE) 
            {
                ...
                if (!colon) { ssl_send_msgex(ssl, MSG_ERROR, "SERVER", nick, "Invalid format", ip); continue; }
                *colon = '\0';
                ...
                time_t ts = time(NULL);
                format_time(ts, time_buf, sizeof(time_buf));
                if (send_private_online_ssl(target, nick, text, false)) 
                {
                    ...
                    ssl_send_msgex(ssl, MSG_SERVER_INFO, "SERVER", nick, confirm, ip);
                } 
                else 
                {
                    ...
                    ssl_send_msgex(ssl, MSG_SERVER_INFO, "SERVER", nick, notice, ip);
                }
                ssl_send_ack(ssl, msg.msg_id, ip);
            }
            else if (msg.type == MSG_LIST) 
            {
                ...
                pthread_mutex_unlock(&clients_mutex);
                ssl_send_msgex(ssl, MSG_SERVER_INFO, "SERVER", nick, list.c_str(), ip);
            }
            else if (msg.type == MSG_HISTORY) 
            {
                ...
                while (offset < hist.size()) 
                {
                    ...
                    buf[chunk] = '\0';
                    ssl_send_msgex(ssl, MSG_HISTORY_DATA, "SERVER", nick, buf, ip);
                    offset += chunk;
                }
            }
            else if (msg.type == MSG_PING) 
            {
                ...
                ssl_send_msgex(ssl, MSG_PONG, "SERVER", nick, id_buf, ip);
        ...
        broadcast_ssl(disc, NULL, "SERVER");
    }
    return NULL;
}
...
int main(int argc, char* argv[]) 
{
    ...
    for (int i = 1; i < argc; i++) 
    {
        ...
        else if (strncmp(argv[i], "--cert=", 7) == 0)    cert_file = argv[i] + 7;
        else if (strncmp(argv[i], "--key=", 6) == 0)     key_file = argv[i] + 6;
    }
    ...
    if (SSL_CTX_use_certificate_file(g_ssl_ctx, cert_file, SSL_FILETYPE_PEM) <= 0) {
        std::cerr << "[Security][CERT] certificate load failed: " << cert_file << std::endl;
        ERR_print_errors_fp(stderr);
        return 1;
    }
    ...
    if (SSL_CTX_use_PrivateKey_file(g_ssl_ctx, key_file, SSL_FILETYPE_PEM) <= 0) {
        std::cerr << "[Security][CERT] private key load failed: " << key_file << std::endl;
        ERR_print_errors_fp(stderr);
        return 1;
    }
