        ...
        if (!recv_msg(fd, msg)) 
        {
            ...
            while (running) 
            {
                ...
                if (connect(new_fd, (sockaddr*)&sa, sizeof(sa)) == 0) 
                {
                    ...
                    {
                        send_msg(new_fd, MSG_AUTH, nick);
                        Message am;
                        if (recv_msg(new_fd, am) && am.type == MSG_SERVER_INFO) 
                        {
                        ...
                        if (am.type == MSG_ERROR) // ошибка авторизации
                        {
                            std::cout << "[ERROR] " << am.payload << std::endl;
                            close(new_fd);
                            running = false;
                            return NULL;
                        }
...
int main()
{
    ...
    if (strlen(nick) == 0) 
    {
        std::cerr << "Nickname can't be empty" << std::endl;
        return 1;
    }
    ...
    send_msg(sockfd, MSG_AUTH, nick);
    if (!recv_msg(sockfd, msg)) 
    {
        std::cerr << "No response to MSG_AUTH" << std::endl;
        close(sockfd); return 1;
    }
    if (msg.type == MSG_ERROR) 
    {
        std::cerr << "[AUTH ERROR]: " << msg.payload << std::endl;
        close(sockfd); return 1;
    }
    ...
    while (running) 
    {
        ...
        else if (line.substr(0, 3) == "/w ") 
        {
            ...
            else 
            {
                ...
                send_msg(fd, MSG_PRIVATE, payload.c_str());
            }
        }
        else if (!line.empty()) 
        {
